# ISC Modular Architecture

> **Goal**: Maximize code sharing across browser, Node.js server, CLI, and future form factors while maintaining clean separation of environment-specific concerns.
...
export const nodeModel: EmbeddingModelAdapter = {
  load: async (modelId) => {
    ...
    // Or use Python bridge to sentence-transformers
  },
...
*This architecture enables ISC to scale from a browser prototype to a multi-platform ecosystem while maintaining a single source of truth for semantic matching logic.*
